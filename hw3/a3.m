...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  log_class_prob = Z2 - repmat(class_normalizer, [size(Z2, 1), 1]);
  A2 = exp(log_class_prob);
  ...
  dW2_classification = (1 / m) .* dZ2 * A1';
  dW1_classification = (1 / m) .* dZ1 * A0';

  dW2_decay = wd_coefficient * W2;
  dW1_decay = wd_coefficient * W1;

  ret.hid_to_class = dW2_classification + dW2_decay;
  ret.input_to_hid = dW1_classification + dW1_decay;
end
