...
while (num_errs > 0)
    ...
    w = update_weights(neg_examples,pos_examples,w,1);
...
function [w] = update_weights(neg_examples, pos_examples, w_current, learning_rate)
%%
...
for i=1:num_pos_examples
    ...
    x = this_case';
    activation = this_case * w;
    % If the output unit incorrectly outputs a 0, add the input to the weight vector
    if (activation < 0)
        w = w + learning_rate * x;
    end
end
